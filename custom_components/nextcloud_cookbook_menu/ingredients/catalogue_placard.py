"""Index of pantry products: kitchen ingredients that keep for more than a week.

Used to file a product added by hand (shopping list, card, voice): if it is listed here,
it joins the pantry (in stock or missing) rather than the "household" section. Names go
through the merge key, so accents, plurals and articles do not matter.
"""
...
PRODUITS_PLACARD: tuple[str, ...] = (
    # Salt, pepper, ground and whole spices
    ...
    # Condiments and sauces that keep
    ...
    # Canned goods and long-life milk
    ...
    "Fruits au sirop",
    # Water and cooking drinks
    "Eau", "Eau gazeuse",
)  # fmt: skip


# Single words that cover their variants ("white rice", "ground cinnamon"). Other single
# words only stand for themselves: "pasta" must not cover "puff pastry".
_FAMILLES = frozenset(
...
def propositions() -> dict[str, str]:
    """Index products by key, in list order (the first name for a key wins)."""
...
# Tolerance for typos ("ras el anout"): names long enough and a very close match only.
LONGUEUR_MIN_APPROCHE = 8
SEUIL_APPROCHE = 0.9


def est_produit_de_placard(cle_produit: str) -> bool:
    """True if the product is a known pantry ingredient, a variant ("organic basmati rice")
    or a close typo ("ras el anout")."""
    if not cle_produit:
        return False
    cles = _cles()
    ...
    # "huile olive vierge extra" starts with "huile olive"; we try from longest to shortest.
